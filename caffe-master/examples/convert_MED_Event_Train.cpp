...
//for (fileid=0;fileid<300;fileid++)
while(!trainFile.eof())
{
...
for(i=0;i<rows*(sampSize/4);i++){
  ...
  dataArray[i]=data;
//std::cout<<"data"<<dataArray[i]<<"\n";
}


//write to the level db
    datum.set_data(dataArray, numFloatsWindow);
    datum.set_label((char)label);
    datum.SerializeToString(&value);
    sprintf(key, "%08d", frameid);
