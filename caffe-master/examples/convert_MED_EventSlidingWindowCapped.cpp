...
//iterate through all the lines
        for(int lineNum=0;lineNum<nSamples && lineNum<5000;++lineNum)
        {
...
void writeToLeveldb(char* db_filename,char* listFile,int windowSize)
{
    ...
    int sizeMFCC;
    int8_t label;
    char key[10],labelChar[2],path[200];
    std::string value;
    // Open leveldb
    leveldb::DB* db;
...
*/
            ...
            {
                if(j==2 | j==3)labelChar[j-2]=fileList[i][j];
                else if(j>4) path[j-5]=fileList[i][j];
            }
    //std::cout<<"path:"<<path<<"\n";
    path[fileList[i].size()-5]='\0';
    string pathStr(path);
    //std::cout<<"pathStr:"<<pathStr<<"\n";

    mfccFileName=pathStr+".htk";
    ...
    //std::cout<<"test"<<"\n";
        ...
        { 
            for(int sampID=0;sampID<windowSize;sampID++)
            {
                ...
                {
                pixels[sampID*numCoeff+dataID]=(char)round(40*mfcc[(windowID)+sampID][dataID]+0);
                }
            }
